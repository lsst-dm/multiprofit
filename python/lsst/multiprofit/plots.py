...
def plot_sersicmix_interp(
    interps: dict[str, tuple[g2f.SersicMixInterpolator, str | tuple]],
    n_ser: np.ndarray,
    **kwargs
    ...
    """
    orders = {name: interp.order for name, (interp, _)  in interps.items()}
    ...
    for i_ord in range(order):
        colors_ord[i_ord] = cmap(i_ord/(order - 1.))
    ...
    n_values = len(n_ser)
    integrals, dintegrals, sigmas, dsigmas = ({name: np.empty((n_values, order)) for name in interps}
                                              for _ in range(4))
    ...
    for idx_row, (yv, yd, yk, y_label) in (
        ...
        for idx_col, y_i, y_prefix in ((0, yv, ''), (1, yd, 'd')):
            ...
            if is_label_col:
                for i_ord in range(order):
                    axis.plot(n_ser_knots, yk[:, i_ord], 'kx',
